...
-- Speicher-System (JSON)
local HttpService = game:GetService("HttpService")
local fileName = "dark_UI_config.json"
...
function Library:Create(table)
    ...
    function tabHandler:Tab(name)
        ...
        function ElementHandler:Toggle(text, callback)
            -- Lade gespeicherten Zustand oder nutze false als Standard
            local savedState = config[text] or false
            ...
            -- Führe Callback sofort aus, falls geladen und true
            ...
            toggle1.MouseButton1Click:Connect(function()
                state = not state
                config[text] = state -- Speichere in lokaler Tabelle
                saveConfig(config)   -- Schreibe in Datei
